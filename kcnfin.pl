...
my $DEFAULT_PID        = 98.1;
my $DEFAULT_DNA_MAX    = 75;
my $DEFAULT_RNA_MAX    = 50; 
...
for ( my $i = 0; $i < scalar @groups; $i++ )
{
   ...
   # print qq[\n#########################GROUP $i: RNA: @RNA\n];
    ...
    for my $rna ( @RNA )
    {
        ...
        my @paths; 
        my $E = $g->edges();        
       # print qq[$rna: DETERMINING ALL POSSIBLE PATHS for $E edges\n];
       ...
       # print qq[$rna FILTERED PATHS\n\n];
        ...
        #my $max_path_c = exists $path_score[2] ? $path_score[2] : undef;
        ...
        # note we keep the two top scoring sub paths for later if either of them exists
        #$path->{b} = defined $max_path_b->{score} ? $max_path_b : 0;
        #$path->{c} = defined $max_path_c->{score} ? $max_path_c : 0; 
        push @MAX_PATHS, $path;  
    }
    ...
    {
        plus_minus(\@MAX_PATHS, \%plus_minus);
        ...
        for my $path ( @MAX_PATHS )
        {
            ...
            one ( \@scaffold, \%plus_minus, $i ); 
            scaffold_paths(\@scaffold, \%plus_minus, $id) if scalar @scaffold; 
        }
    ...
    {   
        ...
        {
            plus_minus(\@component_paths, \%plus_minus);
        ...
        plus_minus ( \@component_paths, \%plus_minus );
        my @pm = grep { exists $plus_minus{plus}{$_} && exists $plus_minus{minus}{$_} } keys %vertices;          
        ...
        if ( scalar @pm )
        {
           # print qq[Possible reverse/foreward transcripts\n];
            ...
            %plus_minus = ();            
            plus_minus ( \@component_paths, \%plus_minus ); 
            @pm = grep { exists $plus_minus{plus}{$_} && exists $plus_minus{minus}{$_} } keys %vertices;
            next if scalar @pm;  
        }
...
{  
    # all sequences marked 'ou' or 'uu' are updated sequences that have not been included in a scaffold because the scaffold was assembled into single sequence  
    print $update_fh qq[>$dna\n$DNA_SEQ{$dna}->{seq}\n] if $DNA_SEQ{$dna}->{status} eq 'u';
}
close $update_fh;
...
$K = grep { $DNA_SEQ{$_}->{status} eq 'ou' } keys %DNA_SEQ; 
print qq[$K primary sequences updated\n];
$K = ();
$K = grep { $DNA_SEQ{$_}->{status} eq 'uu' } keys %DNA_SEQ;
print qq[$K secondary sequences updated\n] if $K;
$K = (); 
$K = grep { $DNA_SEQ{$_}->{status} eq 'so'} keys %DNA_SEQ;
print qq[$K original sequences scaffolded\n];
$K = ();
$K = grep {$DNA_SEQ{$_}->{status} eq 'su' } keys %DNA_SEQ;
print qq[$K updated sequences scaffolded\n];
$K= ();
...
sub one
{
    ...
    return if $connect->vertices() == scalar @$scaffold;
    ...
    else
    {
        ...
        my @e = $connect->edges();
        #print qq[Remaining Edges:\n];
        #print qq[@$_[0] <--> @$_[1]\t] foreach @e; 
        #print qq[\nsource: $source[0]\n]; 
        my $path = DFS ( $source[0], $connect );
        #print qq[PATH:\n];
        #print qq[PATH EXISTS\n] if defined @$path[0];
        my $p = shift @$path;
        push @$scaffold, $_ foreach @$p; 
    }
...
sub overlap 
{
    ...
    {
        my $x = substr ( $query_sequence, $len_q - $align_q - 300 );
        my $y = substr ( $subject_sequence, $align_s, 300 );
        my $xy = $x.$y;
    ...
    for my $path ( @$paths )
    {
        if ( $path->{order} eq 'foreward' )
        {
            for my $dna ( @{$path->{path}} )
            {
                if ( $dna->{strand} eq '+' )
                {
                    $plus_minus->{plus}->{$dna->{id}}++;
                }
                
                elsif ( $dna->{strand} eq '-' )
                {
                    $plus_minus->{minus}->{$dna->{id}}++; 
                }
            }
        }
...
=cut
    ...
    for my $dna ( keys %dd )
    {
        ...
        my @filt = grep { ( ( $_->{match} / $top_match )  < 0.15 ) && $_->{match} < 401 } @rel;
